...
class GraphSageLayer(nn.Module):
    ...
    def __init__(self, in_feats, out_feats, activation, dropout,
                 ...
                 bias=True, dgl_builtin=False, method='convex'):
        super().__init__()
        ...
        if dgl_builtin == False:
            self.nodeapply = NodeApply(in_feats, out_feats, activation, dropout, bias=bias, method=self.method)
...
class NodeApply(nn.Module):
    ...
    def __init__(self, in_feats, out_feats, activation, dropout, bias=True, method='convex'):
        super().__init__()
        self.dropout = nn.Dropout(p=dropout)
    ...
    def concat(self, h, aggre_result):
        ...
        if self.method == 'convex':
            with torch.no_grad(): #TODO: linear.weight
                self.linear.weight.copy_(torch.abs(self.linear.weight))
            # self.linear.weight = torch.abs(self.linear.weight)
        elif self.method == 'concave':
            with torch.no_grad(): #TODO: linear.weight
                self.linear.weight.copy_(-torch.abs(self.linear.weight))
            # self.linear.weight = -torch.abs(self.linear.weight)
